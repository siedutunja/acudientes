...
export default {
  name: 'ConsultaNotasNormalLeonor',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'ESCUELA NORMAL SUPERIOR LEONOR ALVAREZ PINZÓN',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_NORMAL_LEONOR, vigencia: this.vigencia }
        })
