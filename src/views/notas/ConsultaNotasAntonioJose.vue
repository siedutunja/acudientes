...
import BoletinAcudiente from '@/views/boletines/BoletinAntonioJose'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_ANTONIO_JOSE = '54fd7440-fc81-11ec-a1d1-1dc2835404e5'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasAntonioJose',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA ANTONIO JOSÉ SANDOVAL GÓMEZ',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_ANTONIO_JOSE, vigencia: this.vigencia }
        })
