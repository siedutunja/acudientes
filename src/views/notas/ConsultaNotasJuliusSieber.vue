...
import BoletinAcudiente from '@/views/boletines/BoletinJuliusSieber'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_JULIUS_SIEBER = 'bd226a20-fc82-11ec-a1d1-1dc2835404e5'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasJuliusSieber',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA JULIUS SIEBER',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_JULIUS_SIEBER, vigencia: this.vigencia }
        })
