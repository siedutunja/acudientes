...
import BoletinAcudiente from '@/views/boletines/BoletinGustavoRojas'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_GUSTAVO_ROJAS = 'f5529ba0-fcb3-11ec-8267-536b07c743c4'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasGustavoRojas',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA GUSTAVO ROJAS PINILLA',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_GUSTAVO_ROJAS, vigencia: this.vigencia }
        })
