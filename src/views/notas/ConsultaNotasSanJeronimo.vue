...
import BoletinAcudiente from '@/views/boletines/BoletinSanJeronimoEmiliani'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_SAN_JERONIMO = 'acaa36d0-fcb1-11ec-8267-536b07c743c4'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasSanJeronimo',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA SAN JERÓNIMO EMILIANI',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_SAN_JERONIMO, vigencia: this.vigencia }
        })
