...
import BoletinAcudiente from '@/views/boletines/BoletinLibertador'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_LIBERTADOR = '8a1bd1e0-fcb2-11ec-8267-536b07c743c4'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasLibertador',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA LIBERTADOR SIMON BOLIVAR',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_LIBERTADOR, vigencia: this.vigencia }
        })
