...
import BoletinAcudiente from '@/views/boletines/BoletinTecnicoGonzaloSuarez'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_TECNICO_GONZALO_SUAREZ = 'f0491770-fca8-11ec-8267-536b07c743c4'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasTecnicoGonzaloSuarez',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA TÉCNICO GONZÁLO SUÁREZ RENDÓN',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_TECNICO_GONZALO_SUAREZ, vigencia: this.vigencia }
        })
