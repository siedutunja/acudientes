...
import BoletinAcudiente from '@/views/boletines/BoletinGranColombiano'
import BoletinPree from '@/views/boletines/BoletinPree'

const ID_GRAN_COLOMBIANO = '660fa760-fc83-11ec-a1d1-1dc2835404e5'
const OPCION_ACUMULADO = 'ACUMULADO'

export default {
  name: 'ConsultaNotasGranColombiano',
  ...
  data () {
    return {
      ...
      rangoAcumulado: '1-4',
      filtroArea: '',
      nombreInstitucionFallback: 'INSTITUCIÓN EDUCATIVA GIMNASIO GRAN COLOMBIANO',
      escudoInstitucion: ''
    }
  },
  computed: {
    ...
    async cargarConfig () {
      try {
        const resp = await axios.get(CONFIG.ROOT_PATH + 'acudientes/config-evaluacion', {
          params: { idInstitucion: ID_GRAN_COLOMBIANO, vigencia: this.vigencia }
        })
