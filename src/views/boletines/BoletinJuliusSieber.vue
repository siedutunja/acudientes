...
// Portado de academico/src/views/reportes/boletines/BoletinPeriodoJulius.vue
...
//  - Estudiante evaluado como conceptual: no se calcula promedio/puesto general
const COL_DESEM = 7

export default {
  name: 'BoletinJuliusSieber',
  props: {
  ...
  methods: {
    ...
    estructurarNotasPorEstudiante () {
      ...
      this.misNotas.forEach(nota => {
        const {
          ...
          observaciones, pd, C1, C2, C3, concep, conceptual, totalAJ, totalAS
        } = nota
        if (!mapa[estudiante]) {
    ...
    // Desempeño textual: academico (numerico contra umbrales) o comportamiento (letra configurada)
      ...
      return `
        <div class="boletin">
          ...
          <table class="tabla-boletin">
            <thead>
              <tr>
                <th style="width:25%; text-align: left">Promedio: <strong>${this.esConceptual ? '' : this.calcularPromedioGeneral(data)}</strong></th>
                <th style="width:25%; text-align: left">Puesto: <strong>${this.esConceptual ? '' : ((this.puesto || '-') + ' de ' + (this.totalEstudiantes || '-'))}</strong></th>
