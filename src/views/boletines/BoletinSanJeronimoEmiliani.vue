...
// Portado de academico/src/views/reportes/boletines/BoletinPeriodo.vue (plantilla generica del colegio,
...
export default {
  name: 'BoletinSanJeronimoEmiliani',
  props: {
  ...
  computed: {
    ...
    // San Jeronimo Emiliani pondera cada periodo distinto (ej. 20/20/30/30) en vez de 100 parejo -
    // el acumulado se calcula como suma(nota * peso/100), igual que en academico.
    pesosPeriodos () {
      const c = this.config
      return [Number(c.pesoP1) || 0, Number(c.pesoP2) || 0, Number(c.pesoP3) || 0, Number(c.pesoP4) || 0]
    }
  },
  methods: {
    async generarPDF () {
      ...
      if (orden == 99 && this.tipoValComp == 0) {
        ...
        for (const p in asig.periodos) {
          ...
          if (Number.isFinite(nota) && nota > 0) {
            total += (nota * (this.pesosPeriodos[p - 1] || 0)) / 100
            cant++
          }
        }
        ...
        <div class="boletin">
          ...
          <table class="tabla-boletin">
            <thead>
              ...
              <tr>
                ${this.periodosVisibles.map(p => `<th style="font-size: 8px">P${p}[${this.pesosPeriodos[p - 1] || 0}%]</th>`).join('')}
                <th>ACUM</th>
                ${encabezadoDesem}
              </tr>
            </thead>
